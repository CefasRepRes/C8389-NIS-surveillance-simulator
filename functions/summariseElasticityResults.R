...
summariseElasticityResults <- function(results,
  ...
  results$ct_no_detect <- apply(results, 1, function(x) length(unlist(x[[1]])[unlist(x[[1]])]))
  results$pct_no_detect <- apply(results, 1, function(x) length(unlist(x[[1]])[unlist(x[[1]])])/config$num_sim * 100)
